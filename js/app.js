...
async function findElite() {
  ...
  try {
    const species = await fetch(`${POKE_API}pokemon-species/${query}`).then(
      (r) => r.json()
    );
    ...
    const [evoRes, damageData] = await Promise.all([
      ...
      Promise.all(
        elite.types.map((t) => fetch(t.type.url).then((r) => r.json()))
      ),
    ]);
...
function renderFullCard(p, s, evo, damageData) {
  ...
  grid.innerHTML = `
        <article class="pokemon-card" style="border-color: ${color[0]}">
            <div class="card-banner" style="background: linear-gradient(135deg, ${
              color[0]
            }, ${color[1]})">
            ...
            <div class="card-info">
                <div class="title-row">
                    <div><p class="tagline">ID #${
                      p.id
                    } • ${typesHtml}</p><h2 class="name">${p.name.replace(
    /-/g,
    " "
  )}</h2></div>
                ...
                <div class="go-details-grid">
                    <div class="detail-item"><strong>Fraquezas</strong><div>${
                      weakHtml || "Nenhuma"
                    }</div></div>
                    <div class="detail-item"><strong>Resistências</strong><div>${
                      resHtml || "Nenhuma"
                    }</div></div>
                    <div class="detail-item"><strong>Habilidades</strong><span style="text-transform:capitalize">${
                      p.abilities[0].ability.name
                    }</span></div>
                    <div class="detail-item"><strong>Felicidade</strong><span>${
                      s.base_happiness
                    }</span></div>
                </div>
                ...
                <div class="stats-row">
                    <p><strong>HP</strong> ${
                      p.stats[0].base_stat
                    }</p><p><strong>ATK</strong> ${p.stats[1].base_stat}</p>
                    <p><strong>DEF</strong> ${
                      p.stats[2].base_stat
                    }</p><p><strong>SPD</strong> ${p.stats[5].base_stat}</p>
                </div>
            </div>
        </article>`;
}
...
searchInput?.addEventListener(
  "keypress",
  (e) => e.key === "Enter" && ((box.style.display = "none"), findElite())
);
document.addEventListener(
  "click",
  (e) => !e.target.closest(".search-box") && (box.style.display = "none")
);
